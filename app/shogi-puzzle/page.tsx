...
// DnDライブラリのインポートは一旦無効化
import ShogiBoard from '../../components/shogiPuzzle/ShogiBoard';
import { LevelSelector } from '../../components/shogiPuzzle/LevelSelector';
import { GameControls } from '../../components/shogiPuzzle/GameControls';
import { levels } from '../../data/shogiPuzzleLevels';

// メインページコンポーネント
export default function ShogiPuzzlePage() {
  ...
  // 盤面のリセット
  const resetBoard = () => {
    setBoard([...levels[currentLevel].initialBoard]);
    setMoves(0);
    setIsComplete(false);
  };

  // 駒の移動が可能かチェック
  const canMove = (row: number, col: number, targetRow: number, targetCol: number) => {
    // 移動先が盤外の場合
    if (
      targetRow < 0 ||
  ...
  const movePiece = (row: number, col: number, targetRow: number, targetCol: number) => {
    if (!canMove(row, col, targetRow, targetCol)) return;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <div className="bg-white p-4 rounded-lg shadow-md mt-4">
              ...
              <ul className="list-disc pl-5 space-y-2 text-gray-700">
                <li>駒をクリックして選択し、移動先の空きマスをクリック</li>
                <li>または、駒をドラッグして移動先へドロップ</li>
